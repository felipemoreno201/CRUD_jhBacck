 ...
 */
public class ClienteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all clientes").get("/api/clientes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cliente")
                        .post("/api/clientes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"razonSocial\": \"SAMPLE_TEXT\"" + ", \"correo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cliente").get("${new_cliente_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cliente").delete("${new_cliente_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Cliente entity").exec(scn);
