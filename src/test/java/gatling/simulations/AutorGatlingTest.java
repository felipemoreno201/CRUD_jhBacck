 ...
 */
public class AutorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all autors").get("/api/autors").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new autor")
                        .post("/api/autors")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"nombre\": \"SAMPLE_TEXT\"" + ", \"correo\": \"SAMPLE_TEXT\"" + ", \"pais\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_autor_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created autor").get("${new_autor_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created autor").delete("${new_autor_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Autor entity").exec(scn);
