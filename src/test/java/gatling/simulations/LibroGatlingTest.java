 ...
 */
public class LibroGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all libros").get("/api/libros").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new libro")
                        .post("/api/libros")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_libro_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created libro").get("${new_libro_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created libro").delete("${new_libro_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Libro entity").exec(scn);
